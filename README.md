...
```bash
CLI_KEYPAIR_CONNECTION_NAME=your_keypair_connection  # shared keypair conn (JWT + keypair auth)
...
The script reads credentials from your Snow CLI `~/.snowflake/config.toml` (or `connections.toml`) using the `CLI_KEYPAIR_CONNECTION_NAME` connection. External lineage is registered with a JWT generated by `snow connection generate-jwt` (no PAT required).
