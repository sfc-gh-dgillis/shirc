...
if [ $# -lt 1 ]; then
    ...
    echo "Example: $0 tasks/snow-cli/sql/batch-1/001-init-managed.sql"
    exit 1
fi
...
# External volume is NOT required for managed storage mode
REQUIRED_VARS=(
    "DEMO_WAREHOUSE_NAME"
...
echo "Running Snowflake initialization script (managed storage mode)..."
...
# Run snow CLI with templating — no external volume variable needed
snow sql -f "$SQL_FILE" \
...
if [ $? -eq 0 ]; then
    echo ""
    echo "Initialization completed successfully (managed storage)"
