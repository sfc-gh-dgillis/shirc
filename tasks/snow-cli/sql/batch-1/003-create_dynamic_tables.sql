...
USE SCHEMA <% ctx.env.DEMO_SCHEMA_NAME_SILVER %>;
...
CREATE OR REPLACE DYNAMIC ICEBERG TABLE TELEMETRY_ENRICHED
    TARGET_LAG = '1 minute'
    WAREHOUSE = <% ctx.env.DEMO_WAREHOUSE_NAME %>
...
FROM <% ctx.env.DEMO_SCHEMA_NAME_BRONZE %>.VEHICLE_TELEMETRY_STREAM t
LEFT JOIN <% ctx.env.DEMO_SCHEMA_NAME_BRONZE %>.VEHICLE_REGISTRY v ON t.VEHICLE_ID = v.VEHICLE_ID;

-- ============================================
-- Dynamic Table 2: MAINTENANCE_ANALYSIS
-- Purpose: Analyze maintenance logs with extracted fields
-- ============================================
CREATE OR REPLACE DYNAMIC ICEBERG TABLE MAINTENANCE_ANALYSIS
...
FROM <% ctx.env.DEMO_SCHEMA_NAME_BRONZE %>.MAINTENANCE_LOGS m
LEFT JOIN (
    SELECT
    ...
    FROM <% ctx.env.DEMO_SCHEMA_NAME_BRONZE %>.VEHICLE_REGISTRY
...
FROM <% ctx.env.DEMO_SCHEMA_NAME_SILVER %>.TELEMETRY_ENRICHED
...
-- ============================================
CREATE OR REPLACE DYNAMIC ICEBERG TABLE VEHICLE_HEALTH_SCORE
